class RCPBarChart {
  ...
  initVis() {
    ...
    d3.select(vis.config.parentElement)
      ...
      .text(
        "Comparison between Summer and Winter Temperatures for different emission rate"
      );
  ...
  update(currentFilters) {
    let vis = this;
    let categories = ["Average for Historical"];
    ...
    let preparedData = groups
      .map((group) => {
        ...
        return categories.map((cat) => ({
          group,
          category: cat,
          value: categoryData[cat] || 5, // Use 0 as default if undefined
        }));
      })
      .flat();
    ...
    vis.yScale = d3
      .scaleLinear()
      .range([vis.height, 0])
      .domain([0, d3.max(preparedData, (d) => d.value) + 10]);
